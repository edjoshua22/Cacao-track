...
import { getDatabase, ref, push, set, get, query, orderByKey, startAt } from "firebase/database";
...
export default function AddButton({ onBatchCreated }) {
  ...
  const createBatch = async () => {
    ...
    try {
      ...
      const capturesRef = query(ref(db, `captures/${userId}`), limitToLast(50));
      ...
      // OPTIMIZATION: Only fetch last 7 days of history instead of the entire node
      const sevenDaysAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
      const startKey = sevenDaysAgo.toISOString().split('T')[0] + "_00-00-00";
      const historyQuery = query(ref(db, "history"), orderByKey(), startAt(startKey));
      ...
      // Include limited number of images from batch start time onwards to ensure performance
      if (capturesSnapshot.exists()) {
        ...
        const captureEntries = Object.entries(captures)
          ...
          .sort((a, b) => b.imgTimestamp - a.imgTimestamp) // Newest first
          .slice(0, 15); // Process at most 15 images to avoid heavy load
        
        const allImages = [];
        const imagePromises = captureEntries.map(async ({ timestamp, url, imgTimestamp }) => {
          try {
            const result = await inferImage(url);
            const stage = result.stage || 'Unknown';
            const dayKey = result.day || 'day0';
        ...
        const classifiedImages = (await Promise.all(imagePromises));
