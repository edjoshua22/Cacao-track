...
import { ref, onValue, getDatabase, query, limitToLast } from "firebase/database";
...
export default function MonitoringScreen() {
  ...
  useEffect(() => {
    ...
    const processNode = (nodeKey, data, now) => {
      if (!data) return null;
      const dataAge = now - (data.timestamp || now);
      if (dataAge >= 5000) return 'stale';
      return data;
    };

    // Batch all three node subscriptions but write state ONCE per node update
    // to minimise re-renders. Each setsensors call uses functional update so
    // React can batch them when they happen in the same event tick.
    const dht1Unsubscribe = onValue(dht1Ref, (snap) => {
      const now = Date.now();
      const data = snap.exists() ? snap.val() : null;
      const result = processNode('DHT1', data, now);
      if (result && result !== 'stale') {
        setSensors(prev => ({
          ...prev,
          tempDHT1: Number(data.temperature) ?? 0,
          humidDHT1: Number(data.humidity) ?? 0,
          lastDataTime: now,
        }));
        resetDataTimeout();
      } else {
        setSensors(prev => ({ ...prev, tempDHT1: 0, humidDHT1: 0 }));
      }
    });
    ...
    const dht2Unsubscribe = onValue(dht2Ref, (snap) => {
      const now = Date.now();
      const data = snap.exists() ? snap.val() : null;
      const result = processNode('DHT2', data, now);
      if (result && result !== 'stale') {
        setSensors(prev => ({
          ...prev,
          tempDHT2: Number(data.temperature) ?? 0,
          humidDHT2: Number(data.humidity) ?? 0,
          moisture: Number(data.soilMoisture) ?? 0,
          lastUpdate: data.lastUpdate || prev.lastUpdate,
          lastDataTime: now,
        }));
        resetDataTimeout();
      } else {
        setSensors(prev => ({ ...prev, tempDHT2: 0, humidDHT2: 0, moisture: 0 }));
      }
    });
    ...
    const averageUnsubscribe = onValue(averageRef, (snap) => {
      const now = Date.now();
      const data = snap.exists() ? snap.val() : null;
      const result = processNode('Average', data, now);
      if (result && result !== 'stale') {
        setSensors(prev => ({
          ...prev,
          avgTemp: Number(data.temperature) ?? 0,
          avgHumidity: Number(data.humidity) ?? 0,
          lastDataTime: now,
        }));
        resetDataTimeout();
      } else {
        setSensors(prev => ({ ...prev, avgTemp: 0, avgHumidity: 0 }));
      }
    });
    ...
    const sensorUnsubscribe = onValue(sensorRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        if (data != null) {
          let entries = Object.entries(data).map(([_, val]) => ({ ...val }));
        ...
        >
          ...
          >
            ...
            <View style={styles.metricsContainer}>
              ...
              <View style={[styles.metricDivider, { backgroundColor: isDark ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.3)' }]} />
              <View style={styles.metricItem}>
                <Badge type="overall" value={sensors.avgTemp} humidity={sensors.avgHumidity} />
              </View>
            </View>
          </LinearGradient>
              ...
              <View style={styles.cleanSensorGrid}>
                <View style={[styles.cleanSensorCard, { 
                  ...
                  <Text style={[styles.cleanSensorValue, { color: colors.text }]}>
                    {sensors.tempDHT1.toFixed(1)}°C
                  </Text>
                  <Badge type="temperature" value={parseFloat(sensors.tempDHT1)} humidity={sensors.humidDHT1} />
                </View>
                ...
                <View style={[styles.cleanSensorCard, { 
                  ...
                  <Text style={[styles.cleanSensorValue, { color: colors.text }]}>
                    {sensors.tempDHT2.toFixed(1)}°C
                  </Text>
                  <Badge type="temperature" value={parseFloat(sensors.tempDHT2)} humidity={sensors.humidDHT2} />
                </View>
              </View>
            </View>
          </View>
            ...
            <View style={[styles.cleanGroupCard, { 
              ...
              <View style={styles.cleanSensorGrid}>
                <View style={[styles.cleanSensorCard, { 
                  ...
                  <Text style={[styles.cleanSensorValue, { color: colors.text }]}>
                    {sensors.humidDHT1.toFixed(1)}%
                  </Text>
                  <Badge type="humidity" value={parseFloat(sensors.humidDHT1)} humidity={sensors.humidDHT1} />
                </View>
                ...
                <View style={[styles.cleanSensorCard, { 
                  ...
                  <Text style={[styles.cleanSensorValue, { color: colors.text }]}>
                    {sensors.humidDHT2.toFixed(1)}%
                  </Text>
                  <Badge type="humidity" value={parseFloat(sensors.humidDHT2)} humidity={sensors.humidDHT2} />
                </View>
              </View>
            </View>
          </View>
            ...
            <View style={styles.chartCardContent}>
              ...
              <View style={styles.chartContainer}>
                <LineChart 
                  ...
                  tempDHT1Data={chartData.tempDHT1Data}
                  tempDHT2Data={chartData.tempDHT2Data}
                  humidDHT1Data={chartData.humidDHT1Data}
                  humidDHT2Data={chartData.humidDHT2Data}
                  moistureData={chartData.moistData} 
                />
              </View>
            </View>
